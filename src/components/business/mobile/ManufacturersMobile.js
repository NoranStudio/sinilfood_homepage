...
function ManufacturersMobile() {
    ...
    return (
        <div className="manufacturers-section-mobile">
            <h2 className="manufacturers-title-mobile">협력사 및 제조사</h2>
            <div className="sponsors-grid-mobile">
                {sponsors.map((sponsor, index) => (
                    <img key={index} src={sponsor} alt={`sponsor ${index + 1}`} className="sponsor-image-mobile" />
                ))}
            </div>
        </div>
    );
